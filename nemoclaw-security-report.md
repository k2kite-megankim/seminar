...
> - 3.2 바이너리 스코프, 3.3 L7 검사, 3.4 DNS 핀닝, 3.6 내부 호스트 리댁션 → **에이전트 보안** (NemoClaw 독자 설계)
...
### 3.6 정책 컨텍스트 내부 호스트 리댁션 `[에이전트 보안]`

**핵심 파일:** `src/lib/policy/host-redaction.ts`

NemoClaw가 에이전트에게 정책 컨텍스트(허용 엔드포인트 목록 등)를 제공할 때, **내부 네트워크 주소는 에이전트가 볼 수 없도록 필터링**한다. 에이전트가 내부 네트워크 토폴로지를 학습하는 경로를 차단하는 것이 목적이다.

```typescript
// 에이전트에게 노출하기 전에 내부 호스트를 걸러낸다
hostStemsFromEndpoints(rawHosts)
// → { public: ["api.openai.com", ...], redactedCount: 2 }
//   내부 IP 2개는 조용히 제거되어 에이전트 컨텍스트에 포함되지 않는다
```

**차단 범위 (`isInternalHost`):**

| 분류 | 예시 |
|------|------|
| RFC1918 사설 대역 | `10.x`, `172.16-31.x`, `192.168.x` |
| 루프백 | `127.x`, `::1`, `localhost` |
| 링크로컬 | `169.254.x` (클라우드 메타데이터 서버 포함), `fe80::/10` |
| CGNAT | `100.64-127.x` |
| IPv6 ULA | `fc00::/7` |
| 내부 DNS 접미사 | `.local`, `.internal`, `.lan`, `.corp`, `.intranet` 등 |

**`canonicaliseHost()`**: 원본 엔드포인트 값에서 URL 스킴, userinfo(`user:pass@`), 포트, IPv6 대괄호를 제거한 후 검사한다. 파싱에 실패하면 공개 호스트로 통과시키지 않고 리댁션한다 — 미인식 형식은 차단이 기본값.

**3.4 SSRF 방어와의 차이:**

| | 3.4 SSRF 방어 | 3.6 내부 호스트 리댁션 |
|--|--------------|----------------------|
| 시점 | 연결 시도 시 | 컨텍스트 렌더링 시 |
| 목적 | 내부망 접속 차단 | 내부 주소 정보 은닉 |
| 방어 대상 | 실제 연결 | 정보 노출 |
