...
| 레이어 | 적용 시점 | 런타임 변경 |
|--------|-----------|------------|
| 네트워크 정책 | 샌드박스 실행 중 | 가능 (`openshell policy set`) |
| 파일시스템 (Landlock) | 샌드박스 생성 시 | **불가** — 재생성 필요 |
| 프로세스 제어 | 샌드박스 생성 시 | **불가** — 재생성 필요 |
| 게이트웨이 인증 | 이미지 빌드 시 | **불가** — 빌드 시 결정 |
| 추론 라우팅 | 런타임 | 가능 (inference switching 명령) |
...
## 9. 게이트웨이 인증 (OpenClaw 전용)

이 절은 OpenClaw 아키텍처 전용 기능이다. OpenClaw는 로컬 게이트웨이를 통해 외부 API에 접근하는 구조이므로 게이트웨이 인증이 필요하다. Hermes 등 다른 에이전트는 게이트웨이를 사용하지 않으므로 이 절이 적용되지 않는다.

### 9.1 Device Pairing

게이트웨이는 웹 대시보드, CLI, webchat 등 여러 클라이언트가 연결할 수 있는 로컬 서버다. 페어링은 "이 클라이언트가 신뢰할 수 있는 클라이언트인가?"를 최초 연결 시 한 번 검증하는 절차다. 페어링이 없으면 게이트웨이 주소를 아는 누구든 연결할 수 있어, 게이트웨이를 통한 API 호출이나 에이전트 제어가 가능해진다.

게이트웨이에 연결하는 모든 디바이스가 페어링 플로우를 거쳐야 한다. `NEMOCLAW_DISABLE_DEVICE_AUTH=1`은 빌드 타임 설정이며 `openclaw.json`에 구워져 시작 시 해시로 검증된다.

### 9.2 게이트웨이 바인드 주소

기본값은 루프백(`127.0.0.1`)이다. `NEMOCLAW_GATEWAY_BIND_ADDRESS=0.0.0.0`으로 변경 시 네트워크의 다른 호스트가 게이트웨이에 접근할 수 있어 device auth와 함께 검토가 필요하다.

### 9.3 Auto-Pair 허용목록

NemoClaw는 `clientId=openclaw-control-ui` 또는 `clientMode=webchat|cli`이고 `operator.pairing|read|write` 스코프인 경우만 자동 승인한다. 나머지는 거부 후 로깅된다.
