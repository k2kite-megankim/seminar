...
| 플랫폼 | 격리 기술 | 역할 |
|--------|---------|------|
| Linux | Landlock LSM | 커널 파일시스템 접근 제어 — Docker 없이 동작 |
| macOS | Seatbelt (sandbox-exec) | macOS 내장 샌드박스 |
| 공통 | seccomp-notify | 모든 syscall 실시간 중재 |

nono는 컨테이너 런타임 없이 커널 프리미티브만으로 격리한다. Landlock은 Linux 5.13+부터 커널에 내장된 LSM으로, 파일시스템 경로 단위 접근 제어를 제공한다.

**seccomp-notify** (핵심): AI가 커널에 보내는 모든 syscall(`openat`, `connect`, `execve` 등)을 nono supervisor가 실시간으로 가로채 허용/차단을 결정한다. 위반을 기록하는 게 아니라 차단하는 구조다.

**v0.67.1 패치 — `PR_SET_CHILD_SUBREAPER`**: Yama ptrace_scope=1 환경에서 AI가 고아 프로세스를 만들면 seccomp-notify 중재가 끊기는 버그 수정. nono supervisor가 고아 프로세스의 상위자로 등록되어 모든 자식 프로세스 syscall도 중재한다.
...
| 플랫폼 | 격리 기술 | 역할 |
|--------|---------|------|
| Linux | bubblewrap | namespaces 기반 컨테이너 래퍼 |
| macOS | sandbox-exec | macOS 내장 Seatbelt 샌드박스 |
| Windows | WFP + ACE | 커널 네트워크 필터 + 파일시스템 권한 |

**Windows 네이티브 지원 (alpha) — 유일한 차별점:**

비교 대상 중 유일한 네이티브 Windows 샌드박스다. WSL2 없이 동작. nono는 Windows 지원 자체가 없다.
...
격리 깊이는 nono Linux > srt Windows. 그러나 **Windows 환경에서는 srt가 유일한 선택지**다 — nono는 Windows를 지원하지 않는다.
