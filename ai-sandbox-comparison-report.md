...
이 보고서는 두 가지 사용 패턴을 다룬다:

| 구분 | 설명 | 핵심 요구사항 |
|------|------|-------------|
| **제품 내장형** | SaaS/앱 내 AI 기능이 코드 실행·파일 접근 시 격리 | 빠른 콜드 스타트, 낮은 오버헤드, SDK 임베딩, 멀티테넌트 |
...
|--------|---------|------|
| E2B | Apache-2.0 | 오픈소스 |
| Daytona | Apache-2.0 | 오픈소스 |
| Google Agent Sandbox | Apache-2.0 | 오픈소스, CNCF |
| Alibaba OpenSandbox | Apache-2.0 | 오픈소스 |
| NemoClaw | Apache-2.0 | 오픈소스 |
| nono | Apache-2.0 | 오픈소스 |
| Anthropic srt | MIT | 오픈소스 |
| Microsoft AGT | MIT | 오픈소스 |
| Firecracker | Apache-2.0 | 오픈소스, AWS |
...
**GitHub:** github.com/e2b-dev/e2b · **라이선스:** Apache-2.0 · **격리:** Firecracker microVM
**호스트 OS:** 클라우드 관리형 (SDK는 모든 OS)

Firecracker 기반 커널 레벨 격리. 각 요청마다 독립 microVM으로 테넌트 완전 분리. 콜드 스타트 p50 **78ms**, Python/TypeScript/Go SDK.

**보안 강점:** 커널 탈출 불가, 테넌트 간 메모리 완전 분리
**보안 한계:** 크리덴셜 격리 없음(API 키 에이전트 직접 전달), 공급망 보안 없음, 프롬프트 인젝션 방어 없음
**적합:** SaaS 멀티테넌트 AI 코드 실행 내장

---

### 2.2 Daytona

**GitHub:** github.com/daytonaio/daytona · **라이선스:** Apache-2.0 · **격리:** Docker 기본, Kata 옵션
**호스트 OS:** macOS / Linux / Windows

Docker 기본 + Kata Containers 옵션. 콜드 스타트 **27ms** (사전 워밍). HIPAA/SOC2/GDPR 컴플라이언스, 온프레미스 가능.

**보안 강점:** 규제 컴플라이언스, 온프레미스 배포
**보안 한계:** 크리덴셜 격리 없음, Kata 미사용 시 공유 커널
**적합:** 규제 산업 제품 내장 AI

---

### 2.3 Google Agent Sandbox (CNCF)

**GitHub:** github.com/google/agent-sandbox · **라이선스:** Apache-2.0 · **격리:** gVisor / Kata 옵션
**호스트 OS:** K8s가 지원하는 모든 OS

KubeCon NA 2025 CNCF 프로젝트. gVisor syscall 레벨 격리, K8s 네이티브.

**보안 강점:** gVisor syscall 인터셉트, Kata 전환 가능
**보안 한계:** K8s 필수, 크리덴셜 격리 없음
**적합:** K8s 인프라 보유 기업의 AI 코드 실행

---

### 2.4 Alibaba OpenSandbox

**GitHub:** github.com/alibaba/OpenSandbox · **라이선스:** Apache-2.0 · **격리:** Docker / K8s
**호스트 OS:** Docker/K8s가 지원하는 모든 OS

2026년 3월 공개. Coding Agents, GUI Agents, RL Training 지원. 크리덴셜 인젝션 기능 포함.

**보안 강점:** 크리덴셜 인젝션(실제 값 미노출), GUI 에이전트 격리
**보안 한계:** Docker 수준 격리(커널 공유), 크리덴셜 감사 없음
**적합:** 다양한 에이전트 유형 통합 플랫폼

---

## 3. 기업 AI 에이전트 샌드박스

### 3.1 NVIDIA NemoClaw + OpenShell
...
### 3.3 Anthropic sandbox-runtime (srt)
...
### 3.4 Microsoft Agent Governance Toolkit
...
| 기능 | E2B | Daytona | Google Agent Sandbox | OpenSandbox |
|------|-----|---------|---------------------|-------------|
| **격리 방식** | Firecracker microVM | Docker / Kata 옵션 | gVisor / Kata 옵션 | Docker / K8s |
| **커널 탈출 방어** | ✓ | △ Kata 시 ✓ | ✓ | ✗ |
| **콜드 스타트** | 78ms | 27ms | K8s 의존 | 미공개 |
| **크리덴셜 격리** | ✗ | ✗ | ✗ | △ 인젝션만 |
| **SSRF 방어** | ✗ | ✗ | ✗ | ✗ |
| **공급망 보안** | ✗ | ✗ | △ | ✗ |
| **컴플라이언스** | — | HIPAA/SOC2/GDPR | — | — |
| **온프레미스** | 엔터프라이즈 | ✓ | K8s 필요 | ✓ |
| **호스트 OS** | 모든 OS | 모든 OS | 모든 OS | 모든 OS |
...
AI가 생성한 코드를 실행하는가?
    ├─ 멀티테넌트 SaaS?
    │       → E2B (Firecracker, 78ms)
    ├─ 규제 산업 (의료/금융)?
    │       → Daytona (Kata 옵션, HIPAA/SOC2)
    └─ K8s 인프라 보유?
            → Google Agent Sandbox (gVisor/Kata)
...
**시나리오 B: 제품 내장형 경량 샌드박스 구축**

> **Firecracker 직접 활용 또는 E2B 포크 권고**

Firecracker (Apache-2.0, Rust, AWS 유지)는 microVM 레이어만 제공하며 안정적이다. E2B는 Firecracker 위에 SDK와 오케스트레이션을 올린 구조로, 제품 내장 API를 원한다면 E2B 포크가 빠른 시작점이다.

**시나리오 C: 기존 AI 프레임워크에 보안 레이어만 추가**
...
| 상황 | 추천 | 핵심 이유 |
|------|------|----------|
| SaaS에 AI 코드 실행 기능 추가 | **E2B** | Firecracker 격리, 78ms |
| 규제 산업 제품 내장 | **Daytona** | Kata 옵션, HIPAA/SOC2 |
| K8s 기반 AI 코드 실행 | **Google Agent Sandbox** | gVisor/Kata, K8s 네이티브 |
| 기업 에이전트 보안 플랫폼 포크 | **nono** | 보안 설계 최우수, NVIDIA 종속 없음 |
| 제품 내장 microVM 포크 | **E2B / Firecracker** | Apache-2.0, 검증된 격리 |
...
- Google Agent Sandbox: github.com/google/agent-sandbox
